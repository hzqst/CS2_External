...
void Cheats::Menu()
{
	...
	ImGui::Begin("Menu",nullptr,ImGuiWindowFlags_AlwaysAutoResize);
	{
		...
		{
			Gui.MyCheckBox("Spotted", &MenuConfig::ShowSpotted);
...
void Cheats::Run()
{
	...
	for (int i = 0; i < 64; i++)
	{
		...
		if (!ProcessMgr.ReadMemory<DWORD64>(gGame.GetEntityListEntry() + (i + 1) * gGame.GetEntityListEntrySize(), EntityAddress))
			continue;
	...
	/*	for (int BoneIndex = 0; BoneIndex < Entity.BoneData.BonePosList.size(); BoneIndex++)
		...
		if (MenuConfig::ShowSpotted && Entity.IsAlive())
		{
			Entity.Pawn.SetSpotted(true);
		}
